...
* Status (6/13/2025)
I got bit by the bug again.  After all the magic tricks and backflips required to get
us to like a 90% embedded clojure environment using a combination of reader macros,
hacking the sbcl evaluator, heavy metaprogramming to enable lexical analysis, and
all sorts of unsavory (but fun to learn) stuff in Common Lisp, I decided to get back
to the original approach I had avoided (due to exploring the bounds of embedding).

** Separation of concerns
I leveraged extant implementations of protocols, variadic functions, sequences,
persistent data structures, etc. to effectively bootstrap a clojure runtime in
Common Lisp.   This time, instead of trying to leverage the ambient facilities in
CL for embedding a new language (and their surprising limitations), we instead 
leverage the flexibility of CL to implement an equivalent of clojure.lang.RT
or the cljs equivalent.  This way, we can focus on providing native CL 
implementations of Clojure concepts, and defer the reader and analyzer work
to tools.reader and tools.analyzer respectively. 

** We already have most of clojure.core  and the runtime
I was able to leverage prior work with some cleanup (removing reader, eval,
and lexical analyzer tricks) to get a common clojure runtime.  So we have
defprotocol, deftype, def, defn, reify, loop*, fn*, etc. and most of the 
primitives out of the box.  I'll fill in gaps with library support (like using
clj-con for concurrency stuff, cl-hamt, cl-murmurhash, etc).

*** Data Literals Already Print 
We already have (on the printing side) organic printing of clojure
data structures (or the proxies in place).  So we get a nice quality
of life when building out the clojure runtime in common lisp.  That
then translates directly into clojure printing.  Printing is probably
the easiest piece of this and it's already done.

*** Let the Reader Read, not Common Lisp Readtable
Reader macros are cool, but they start to get funky when you try to nest
new data literals into lexical forms like `let`.  I resorted to a bunch of
code walking and parsing to jump back and forth just to get vectors and maps
working in all contexts.  Even keyword-as-function access ended up being a 
semi-surmountable hurdle, but not without its problems.

So no more bending the readtable and other shenanigans.  We go back
to porting clojure.tools.reader to our clojure runtime in common lisp.

The good news is we have like 99% of what we need that's now "native" 
in CL (via macros), we just need to translate the reader literals and friends
over, and fill out platform-specific gaps (e.g. java classes or js classes, 
we'll probably leverage the cljs implementation since it's already protocol
based).

I hope that by porting the existing official readers, we get all the corner
cases solved for us (like metadata, number parsing, etc). We also
get reader conditionals (including platform junk).

**** Some literals have different semantics
In clojure, symbols and keys can be qualified or unqualified.
In CL, they are automatically "qualified" in the sense that the
reader wants to create them local to the current package.  You 
can get unqualified with some effort, but it's enough of an
ass pain to work around that it's best we lift our own keys/symbols/ns
out of the environment and translate that to CL at compile time.

This will let us encode the clojure semantics inside the clojure
compilation/evaluation context, and let the reader build out actual
clojure literals directly (e.g. packing on meta and the like where we
want to, reading unqualified stuff).

*** Leverage the Analzyer, Emit CL
Since our clojure runtime is pretty robust and maps directly (including
most of the core libs) to the clojure.core implementation from cljs (protocol based), 
we have more than enough to build out the analyzer.  We also have a dually 
flexible environment for porting the existing clj/cljs emitters into compatible
clojure CL runtime forms.  We can leverage substantial metaprogramming facilities
here to trivialize codegen.

This opens up some opportunities down the road for additional optimizing
passes.  Notably, SBCL already does a bunch for us (quite noisly) like
pruning unused code.  The compiler is badass and we can expose more of that
to clojure code directly (down the road).

*** Parallel Construction
We implement our own vars, keys, symbols, and namespaces going forward.
There may be a way to embed better with e.g. the CL package system (which I originally
tried), but for now we'll sidestep satisfying CL and just do clojure in its
own environment.

Thankfully these aren't too hard to implement, and there are at least some
facilities present in the CL ecosystem to make it easier to do so.

From an interop perspective, we should be able to trivially embed CL forms
that can be resolved natively, and I think exposing clojure namespaces and 
symbols should be similarly easy.  Since clojure functions are common lisp
function/generic functions, and clojure classes are CLOS classes, so long
as we can resolve it we should be able to use it.

There is a question about maintaining or mirroring clojure namespaces in 
CL packages. I'll think about that a bit down the road.

** Current Trajectory
I envision we'll have something like cljs (or the jvm implementation) 
where our "triangle on screen moment" is when we have something like 

#+BEGIN_SRC  
(print-clojure 
  (eval-clojure 
    (read-clojure "(let [m {:x [1 2 #{}]}] (-> m :x last  (conj :blah)))")))
;;yields #{:blah}
#+END_SRC

where we're reading, analyzing, evaling (resolve + apply), and printing
everything across the common-lisp->clj bridge.  Then go the opposite
direction (e.g. with a #cl reader dispatch).

- Starting work on porting the reader now.

- It's possible to write non-trivial "clojure" programs in CL as is, but
  without reader support (which is sorely missed) and some semantics like
  keys-as-functions.
  - You get loop/recur, fn/recur, def, defn, metadata, sequences, vectors, 
    a weak cow-map (replaced later), and a good chunk of the core library
    (with more ported as needed), atoms, and other stuff added daily.
...
values in quoted literals.  I did some naive variant that's bone-headed in hindsight,
where I tried to eval stuff at read-time to build an actual non-sexp literal datastructure.
Turns out, this is not what Clojure does.  Clojure does depart from CL's quasiquotation 
(via =syntax-quote=), in that for quoted literals, it emits the actual sexpr construction
of the literal.  This is similar to my solution for working around lexically scoped
literals.  The only challenge in SBCL is that the =`= reader macro defaults to emitting
a =quasiquote= form.  So if we leverage the built-in stuff, we get the quoted form
of the sexpr vector constructors expanded from the original quasiquoted literal.

The solution is to hack the =`= macro and rebind it to something that's smart enough
to detect that we're actually quoting a literal.  From there, we should just emit the
expanded quasiquote result without the =quasiquote= form preceding it.  Or otherwise, 
...
Signficant overhaul to the old (wrong) qq infrastructure.  Eliminated all
readtime eval stuff, we now emit sexprs consistent with Clojure's implementation (slightly
different in form though).

We now have a cleaner quasiquotation model that's extensible as well.

*** Generalized usage of sequence lib
Ensured the sequences lib is portable enough to be used, sicne it shows up
quite a bit in clojure.core bootstrapping (e.g. macrology).  Need to define
additional core functions that are seq friendly or coercing.

*** Destructuring
Destructuring is pretty fundamental and shows up in a lot of places.
So common forms like =loop=, =for=, =let=, =fn=, =defmacro= and friends
all leverage it.  We should be able to fairly straightforwardly port it
from clojure.core.

**** Revisited Loop/Recur and variadic version
During porting of more core library functions over, I exposed bugs
in the extant labels/macrolet based version of function implementation.

Did some experimenting and switched to the earlier loop-focused
=with-recur= macro.  Did some more testing and experimentation,
and switch over to inlining arity bodies for multiple-arity
function definitions.  So named functions work, recur works,
=loop= (without destructuring) is implemented (over =loop*=).

Our functions now work fine with =lazy-seq= where before
they were creating infinite loops and later not
recognizing function symbols for self recur.


*** ex-info
En route to =destructure=, I implemented exception handling
via =ex-info= and friends.  =try-catch-finally= is a macro
away.  

*** TCO detecting false positives
Need to revisit the tail cail detection code that compiles
=with-recur= invocations, since we seem to be mistaking
simple =with-let= and =if-let= forms for non-tail
calls for some reason.  For now, I've got warnings
rendering, where in reality we want an error to signal (prior
behavior).

*** Rudimentary Parity
We're approaching basic functionality where I can just
copy/paste source from clojure.core and have it work without
issue. It turns out, this is a great way to explore the
range of language features from the ground up and implement
as needed.  It might be possible to demo some advent of code
solutions in clojure in common lisp before too long...


*** try 
Defined try-catch-finally semantics from clojure.core/try in common-utils:try, which
clclojure.base exports.  Trivial to implement using =handler-case= and =restart-case=.

*** ~ and ~@
Realized that =sb-impl::comma-charmacro= handles this and splicing (@ form), so all 
you have to do is add another reader macro binding for =~= using the same as =,=.

*** cond
Implemented clojure's cleaner version of =cond=.  Found a subtle error regarding
the definition and use of =cons=.  Since we shadow =common-lisp:cons=, we ended up
with an infinite pingpong of the protocol extension of =-conj= for =cons= types.
Fix was to ensure we use =common-lisp:cons= for all necessary non-seq cons cell
methods.
