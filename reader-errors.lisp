...
(defpackage cljs.tools.reader.impl.errors
  ...
  (:shadowing-import-from :clclojure.base
   ...
   :zero? :when-let :assoc :dissoc :get :conj :disj :merge :with-meta :meta :subs :apply :name :count)  
  (:shadowing-import-from :cljs.tools.reader.impl.utils :char :whitespace? :newline?)
  (:shadow :read-char :peek-char :read-line :reader-error)
  (:local-nicknames (:base  :clclojure.base)
                    (:types :cljs.tools.reader.impl.reader-types)
                    (:i     :cljs.tools.reader.impl.inspect)))

(in-package :cljs.tools.reader.impl.errors)
(named-readtables:in-readtable clj-re:readtable) ;;maybe unnecessary.

(defn ex-details
  (rdr ex-type)
  (let (details (hash-map  :type :reader-exception
                           :ex-kind ex-type))
...
(defn throw-ex (rdr ex-type & msg)
  (let (details (ex-details rdr ex-type)
        file    (get details :file)
        line    (get details :line)
        col     (get details :col)
...
;;we'll ignore letfn for the moment.

(defn duplicate-keys-error (msg coll)
  (let (dups (base::->> coll
                 (base:frequencies)
                 (base:filter (base:fn ((k n)) (> n 1)))
                 (base:map base:first)))
    (base:str msg
           (when (> (count dups) 1) "s")
           ": " (base:apply str  (base:interpose ", " dups)))))

(defn throw-dup-keys (rdr kind ks)
  (reader-error
   rdr
   (duplicate-keys-error
    (str (string-upcase (name kind)) " literal contains duplicate key")
    ks)))
